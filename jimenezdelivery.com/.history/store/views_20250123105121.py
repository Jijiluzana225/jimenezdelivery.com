...
def store_products(request, store_id):
    ...
    product = Product.objects.filter(store=store, product__active=False)
    items = Item.objects.filter(store=store, product__active=False).select_related('product')
    return render(request, 'store/store_products.html', {'store': store, 'items': items, 'product':product})
